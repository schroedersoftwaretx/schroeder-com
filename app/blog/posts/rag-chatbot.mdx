...
summary: "How I built the backend for a RAG chatbot in my senior design class: a Selenium scraper, a Firestore ingest layer, hybrid retrieval with a cross-encoder reranker, and an async FastAPI service. Then everything that broke quietly instead of loudly, including a bug I only found while writing this post."
---
...
<Callout title="On caching the right thing">
  Notice the `_build_bm25(contents)` call on the cache-hit path. The dense vectors get cached because they're expensive. The BM25 index doesn't, because rebuilding it from raw text is nearly free, but it still has to be rebuilt or you silently fall back to vector-only search. Caching is easy, remembering everything that has to be reconstructed alongside the cache is the punishing part.
</Callout>
